...
\resumeSubheading{
    \href{https://github.com/AaLexUser/Functional-programming-4} % Placeholder - PLEASE UPDATE LINK
    {Retrieval-augmented generation (RAG) система}}{\textit{Декабрь 2024}}{Clojure, Pgvector, LangChain4j, Docker}{ } % Placeholder - PLEASE UPDATE DATES
\resumeItemListStart
\resumeItemNoBold{Разработал \textbf{Retrieval-Augmented Generation (RAG)} систему на \textbf{Clojure} для ответов на вопросы на основе документов.}
\resumeItemNoBold{Использовал \textbf{Pgvector} в качестве векторной базы данных и \textbf{Ollama} для локального запуска \textbf{LLM}.}
\resumeItemNoBold{Применил \textbf{LangChain4j} для интеграции с \textbf{LLM} (модель \textbf{nomic-embed-text} для эмбеддингов) и управления процессом RAG.}
\resumeItemNoBold{Реализовал \textbf{RESTful API} (используя \textbf{Ring, Compojure, Jetty}) и веб-интерфейс на \textbf{ClojureScript} с \textbf{Reagent} и \textbf{Reagent Material UI}.}
\resumeItemNoBold{Обеспечил развертывание с помощью \textbf{Docker} и \textbf{Docker Compose}.}
\resumeItemListEnd
